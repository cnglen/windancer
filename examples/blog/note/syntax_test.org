...
:LAST_MODIFIED: 2026-04-11T08:57:20+0800
...
* DONE Elements
CLOSED: [2026-04-07 Tue 18:58]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-04-07 Tue 18:58] \\
  all element is documented. Nome some elements are not supported now.
...
The following drawer is NOT a property drawer, since property drawer are attached to a *heading* or *inlinetask*, or in zeroth section. And its content will not appear in exported html, too.
...
*** Footnote Defintions
:PROPERTIES:
:CUSTOM_ID: footnote_definition_headline
:END:
...
#+begin_note
The definitons *inline* and *anonymous* footnote are exported with other kind of footnotes together(i.e, into the last part named "Footnotes" of html page)
#+end_note
...
:foo:      bar
:key:      value
:CUSTOM_ID: property_drawer_heading
:END:
...
*** Table
:PROPERTIES:
:custom_id: table_heading_line
:END:

Table *alignment* and *colgroup* are supported.

{{{color(red, FIXME)}}}: [0/1]
- [ ] table with multiple formulas, use which formula?

#+CAPTION: Table with columns aligned differently
| Name       |       Phone | Age  |   as   |  xx | yy  |
|------------+-------------+------+--------+-----+-----|
|            |       <r10> | <l>  |  <c>   |     |     |
| Peter Jack |         123 | 2    |   4    |   a | b   |
| Anna       |       54321 | 125  |  999   |  ff | gg  |
| align demo | right align | left | center | <r> | afd |
|            |             |      |        | <r> |     |
|------------+-------------+------+--------+-----+-----|
| bad group  |           < | >    |        |     |     |
| !          |           < | >    |        |     |     |
...
#+CAPTION: table with multiple formulas
|   | x |    z |
|---+---+------|
| ! | x |   z1 |
| # | 1 |  300 |
...
table cell's CONTENT can contain, see [[#objects_overview_heading][objects oveview]].
...
The following it NOT a comment line, since no spaces flollwing ​~#~​:
#a
...
The follwoing is not a horizontal rule, since it only contains 4 dashes:
...
Node properties can ONLY EXIST in [[#property_drawer_heading][property drawers]].
...
Table rows can ONLY EXIST in [[#table_heading_line][tables]] with Org type.

* DONE Objects
CLOSED: [2026-04-27 Mon 19:36]
:PROPERTIES:
:CUSTOM_ID: objects_overview_heading
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-04-27 Mon 19:36]
- State "TODO"       from              [2026-04-04 Sat 18:03]
:END:

#+CAPTION[object in table]: table with objects
#+NAME: table_with_objects
...
** DONE Footnote References
CLOSED: [2026-04-11 Sat 17:51]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-04-11 Sat 17:51]
- State "TODO"       from              [2026-04-04 Sat 17:37]
:END:

This is a org [fn:org-mode] file.

The inline footnote[fn:inline_footnote]  defined in [[#footnote_definition_headline][Footnode Defintion Heading]]  is referenced again here.


** TODO Citation
:PROPERTIES:
:CUSTOM_ID: citation_headline
:END:
:LOGBOOK:
- State "TODO"       from              [2026-04-11 Sat 19:25]
:END:
...
:LOGBOOK:
- State "TODO"       from              [2026-04-11 Sat 19:29]
:END:

Citation references are only found within [[#citation_headline][Citiation]]s


** TODO Inline Babel Calls
:LOGBOOK:
- State "TODO"       from              [2026-04-11 Sat 19:31]
:END:
...
** DONE Inline Source Blocks
CLOSED: [2026-04-11 Sat 19:43]
:LOGBOOK:
- State "DONE"       from "TODO"       [2026-04-11 Sat 19:43]
...
** DONE Links
CLOSED: [2026-04-27 Mon 18:55]
:PROPERTIES:
:ID:       e30bb0f4-a323-4dce-8f6b-2f3b060e6dd0
:END:
:LOGBOOK:
- State "DONE"       from              [2026-04-27 Mon 18:55]
:END:

- regular link:
  - with description [[https://www.baidu.com][baidu]];
  - without description [[https://google.com]]
  - description with objects [[http://foo.bar][\alpha *bold* $a+b$ foo]];
  - fuzzy
    - internal [[target]]
    - relative link [[./editor/emacs.html][emacs]]
...
radio link: define four radio targets:
- <<<*bold* target>>>
- <<<my radio target>>>
- <<<\alpha>>>
- <<<$\alpha$>>>

then we have *bold* target, my radio target, $\alpha$ and \alpha.
...
={{{property(name)}}}= and ​={{{n}}}=​  *{{{color(red, NOT supported)}}}* now


| macro name        | arguments     | raw reference                                  | rendered value                                   | kind                                                 | supported |
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
| title             |               | ={{{title}}}=                                  | {{{title}}}                                      | keyword(title)                                       | Yes       |
| author            |               | ={{{author}}}=                                 | {{{author}}}                                     | keyword(author)                                      | Yes       |
| author            |               | ={{{author()}}}=                               | {{{author}}}                                     | keyword(author)                                      | Yes       |
| email             |               | ={{{email}}}=                                  | {{{email}}}                                      | keyword(email)                                       | Yes       |
| keyword           | NAME          | ={{{keyword(project)}}}=                       | {{{keyword(project)}}}                           | keyword(NAME)                                        | Yes       |
| date              |               | ={{{date}}}=                                   | {{{date}}}                                       | keyword(date)                                        | Yes       |
| date              | FORMAT        | ={{{date(%Y-%m-%dT%H:%M:%S%z)}}}=              | {{{date(%Y-%m-%dT%H:%M:%S%z)}}}                  | format keyword(DATE) if it's a signle timestamp      | Yes       |
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
| time              | FORMAT        | ={{{time(%Y-%m-%dT%H:%M:%S%z)}}}=              | {{{time(%Y-%m-%dT%H:%M:%S%z)}}}                  | export time                                          | Yes       |
| modification-time | FORMAT        | ={{{modification-time(%Y-%m-%dT%H:%M:%S%z)}}}= | {{{modification-time(%Y-%m-%dT%H:%M:%S%z)}}}     | modification time                                    | Yes       |
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
| input-file        |               | ={{{input-file}}}=                             | {{{input-file}}}                                 | filename of the exported file                        | Yes       |
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
...
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
| color             |               | ={{{color(red, red text)}}}=                   | {{{color(red, red text)}}}                       | by =#+macro:=                                        | Yes       |
|-------------------+---------------+------------------------------------------------+--------------------------------------------------+------------------------------------------------------+-----------|
| results           | value         | ={{{results(=None=)}}}=                        | src_python{print("hello")} {{{results(=None=)}}} | inline source block                                  | Yes       |

{{{color(red, two line
of red)}}}


** DONE Targets and Radio Targets
CLOSED: [2026-04-12 Sun 17:38]
:LOGBOOK:
- State "DONE"       from              [2026-04-12 Sun 17:38]
:END:
*** Targets

<<yes a target>> this is a target named ​~yes a target~​
...
Here we refer to item [[yes a target][descript of target]]. (C-c C-o)

*** radio target

define a radio target: <<</italic/ target>>>, then reference a target /italic/ target




** DONE Statistics Cookies
CLOSED: [2026-04-27 Mon 18:44]
:LOGBOOK:
- State "DONE"       from              [2026-04-27 Mon 18:44]
:END:
...
** DONE Subscript and Superscript
CLOSED: [2026-04-27 Mon 17:29]
:LOGBOOK:
- State "DONE"       from              [2026-04-27 Mon 17:29]
:END:

In the default configuration, ​={}=​ is required for subscript and super script. The option can be configured by:
#+begin_src toml
  [compiler.parser]
  # Three value supported:
  # - brace: only _{}/^{} is recognized as subscript and superscript
  # - nil: disable subscript and superscript
  # - true: {}/()/* https://orgmode.org/worg/org-syntax.html#Subscript_and_Superscript
  org_use_sub_superscripts = "true"  
#+end_src

*** Superscript

- simple :: an expression enclosed in curly brackets
  - a^{34}
  - a^{(34)} 
  - a^{b{3}}
  - a^{b^{c}^{d}^{e}}
...
- complex with object
  ...
  - a^{385+3 [[https://baidu.com][baidu]]}
- Super script is enabled only if ​=org_use_sub_superscripts=​ is set to "true":
  - a^(c+4)
  - foo^abc...
  - a^+34
  - a^-12,889,,,,78.3\a,
...
The display style can be toggled by ​=org-toggle-pretty-entities=​ or =C-c C-x=
...
- a single asterisk:
  ...
  - __{*},
  - ,_{*}
- Subscript is enabled only if ​=org_use_sub_superscripts=​ is set to "true":
  - a_+34,
  - a_-12,889,78.3\a,
  - 10_24,
  - 10_tau,
...
See [[table_with_objects][this table]]
...
*_~inner-most-with-underline-bold~_*

/_~inner-most-with-underline-italic~_/
...
***a bold*** : 3 bold matched

**a bold** : 2 bold mathced
...
这是​*粗体*​，​/斜体/​, ​_下划线_​，​*/粗斜体/*​，​+横滑线+​。


** DONE Plain Text
CLOSED: [2026-04-27 Mon 19:32]
:LOGBOOK:
- State "DONE"       from              [2026-04-27 Mon 19:32]
:END:
